...
You could even mount network volumes, for all Docker cares!
...
## Secrets
Docker Secrets are **a way to give containers access to relevant information securely**, without having to rely on environment variables, or worse, direct copying of files in images.
This enhances security: environment variables are broadcast to every process in a container, even child processes.
Secrets alleviate this by being stored in files that process have to **manually read from**.
