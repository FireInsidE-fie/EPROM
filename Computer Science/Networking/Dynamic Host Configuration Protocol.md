...
Any DHCP server that might hear it on the local network will then **answer with a DHCP *Offer***, containing an IP address and the other **network configuration the new host needs**.
...
Finally, the DHCP server sends a final acknowledgment (*ACK*) confirming the new configuration; the new device can now use the IP address for a specified period of time.
