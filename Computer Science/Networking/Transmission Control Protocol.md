...
- **Acknowledgment Number**: After a piece of data has been assigned a sequence number, the number for the next piece will be the sequence number + 1. See below for details.
...
...
# TCP Slow-Start
When a new connection is created, **it starts at a slow speed (a small congestion window)**.
As time goes on and more data is transmitted successfully, it *tunes* itself to get faster and faster.
This is why nowadays most protocols running over TCP **keep connections alive** to avoid this cost each time they want to transmit something.
# Resources
