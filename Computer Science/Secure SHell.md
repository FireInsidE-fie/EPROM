...
# Security
SSH uses [[Asymmetric Encryption]] to secure connections, with public and private keys.
...
The `.ssh/authorized-keys` lists allowed keys that can be used to connected to the local server. Adding a key inside of it can be a useful back door when penetration testing.